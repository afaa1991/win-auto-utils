...
impl ShellcodeBuilder {
    ...
    pub fn pop_all_registers(&mut self) -> &mut Self {
        match self.arch {
            ...
            Architecture::X64 => {
                ...
                for reg in [0x5F, 0x5E, 0x5D, 0x5C, 0x5B, 0x5A, 0x59, 0x58]
                    .iter()
                    .rev()
                {
    ...
    pub fn mov_mem32_from_reg(&mut self, address: usize, reg_index: u8) -> &mut Self {
        if reg_index > 7 {
            panic!(
                "32-bit registers only support indices 0-7 (EAX/RCX to EDI/RDI), got {}",
                reg_index
            );
        }
        ...
        match self.arch {
            ...
            Architecture::X64 => {
                ...
                self.bytes.push(0xBB); // MOV R11, imm64
                self.bytes
                    .extend_from_slice(&(address as u64).to_le_bytes());
