...
fn test_noop_throughput() {
    ...
    assert!(
        ips > 1_000_000.0,
        "No-op throughput too low: {:.0} IPS",
        ips
    );
...
fn test_sleep_throughput() {
    ...
    assert!(
        overhead_per_instr < 1_000_000.0,
        "Sleep overhead too high: {:.2} ns",
        overhead_per_instr
    );
...
fn test_keyboard_throughput() {
    ...
    assert!(ips > 1_000.0, "Keyboard throughput too low: {:.0} IPS", ips);
...
fn test_mouse_throughput() {
    ...
    assert!(ips > 500.0, "Mouse throughput too low: {:.0} IPS", ips);
...
fn test_mixed_instruction_comparison() {
    ...
    println!(
        "  Time: {:?}, IPS: {:.0}, Per iteration: {} μs",
        elapsed_original,
        ips_original,
        elapsed_original.as_micros() / iterations as u128
    );
    ...
    let script_optimized_same_sleep = format!(
        r#"loop {}
    key A
    move 100 200
    click
    ...
    println!(
        "  Time: {:?}, IPS: {:.0}, Per iteration: {} μs",
        elapsed_optimized_same,
        ips_optimized_same,
        elapsed_optimized_same.as_micros() / iterations as u128
    );
    ...
    println!(
        ...
        elapsed_no_sleep,
        ips_no_sleep,
        elapsed_no_sleep.as_micros() / iterations as u128
    );
    ...
    let improvement_same_sleep = if elapsed_original > elapsed_optimized_same {
        ...
        format!(
            "✅ Improved by {} μs/iter ({:.1}%)",
            saved / iterations as u128,
            percent
        )
    } else {
    ...
    println!(
        "  3. Pure instruction performance (no sleep): {:.0} IPS",
        ips_no_sleep
    );
...
fn test_nested_loop_overhead() {
    ...
    assert!(
        ips > 500_000.0,
        "Nested loop throughput too low: {:.0} IPS",
        ips
    );
...
fn test_compiled_script_reuse_benefit() {
    ...
    let savings_percent = if total_time_no_cache.as_nanos() > 0 {
        ((total_time_no_cache.as_nanos() - total_time_with_cache.as_nanos()) as f64
            / total_time_no_cache.as_nanos() as f64)
            * 100.0
    } else {
        0.0
    };
    ...
    println!(
        "Total time without cache ({} runs): {:?}",
        runs + 1,
        total_time_no_cache
    );
    println!(
        "Total time with cache ({} runs): {:?}",
        runs + 1,
        total_time_with_cache
    );
...
fn test_instruction_dispatch_overhead() {
    ...
    for &size in &test_sizes {
        ...
        println!(
            "Size: {:>6} | Time: {:>12?} | IPS: {:>12.0} | Per instr: {:>8.2} ns",
...
fn test_setcursorpos_vs_sendinput_performance() {
    ...
    println!(
        "  Per call: {:.2} ns",
        time_setcursorpos.as_nanos() as f64 / iterations as f64
    );
    ...
    println!(
        "  Per call: {:.2} ns",
        time_sendinput.as_nanos() as f64 / iterations as f64
    );
    ...
    println!(
        "  Optimized script engine is {:.1}x faster than raw SetCursorPos",
        speedup
    );
