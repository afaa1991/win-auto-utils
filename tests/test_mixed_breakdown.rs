...
fn test_mixed_instruction_breakdown() {
    ...
    println!("  Expected (sum): {:?} (key + move + click + sleep)", 
             elapsed_key + elapsed_move + elapsed_click + elapsed_sleep);
    
    // Analysis
    println!("\n=== Analysis ===");
    let estimated_os_calls = elapsed_key.as_micros() + elapsed_move.as_micros() + elapsed_click.as_micros();
    ...
    println!("Difference: {} μs (likely scheduling/context switching overhead)\n", 
